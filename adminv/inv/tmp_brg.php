...
if ($_POST['btnSMP'])
	...
	do
		...
		if ($tcc != 0)
			{
			//update
			mysql_query("UPDATE inv_brg_program SET jml = '$yuhux' ".
							"WHERE kd_brg = '$brgkd' ".
							"AND kd_program = '$kea2x'");
			}
			...
			//insert
			$xyz = md5("$x$i");
			mysql_query("INSERT INTO inv_brg_program(kd, kd_brg, kd_program, jml) VALUES ".
...
if ($s == "edit")
	...
	do
		...
		//jml-nya...
		$qkeb = mysql_query("SELECT * FROM inv_brg_program ".
								"WHERE kd_brg = '$brgkd' ".
								"AND kd_program = '$kea_kd'");
