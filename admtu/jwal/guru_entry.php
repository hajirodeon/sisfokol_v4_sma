...
if ($_POST['btnSMP'])
	...
	if ((empty($kprp)) OR (empty($hari)) OR (empty($jam)) OR (empty($lama)))
		...
		$rpx_keakd = nosql($rrpx['kd_program']);
		...
		//jika tubrukan
			...
			for($j=1;$j<=$lama;$j++)
				...
				//query
				mysql_query("INSERT INTO jadwal(kd, kd_tapel, kd_smt, kd_program, kd_kelas, ".
...
do
	...
	echo '<option value="'.$kui_kd.'">'.$kui_program.' / '.$kui_kelas.' / '.$kui_ruang.' / '.$kui_pel.'</option>';
	}
